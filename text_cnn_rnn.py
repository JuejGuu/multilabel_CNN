	...
	def __init__(self, embedding_mat, non_static, hidden_unit, sequence_length, max_pool_size,num_classes, embedding_size, filter_sizes, num_filters, l2_reg_lambda=0.0):
		...
		for i, filter_size in enumerate(filter_sizes):
			with tf.name_scope('conv-maxpool-%s' % filter_size):
				...
				tf.summary.histogram('conv_layer_output', h)
				...
				tf.summary.histogram('maxpooling_layer_output', pooled)
		...
		with tf.name_scope('output'):
			...
			tf.summary.histogram('output_scores', self.scores)
                
			self.predictions = tf.argmax(self.scores, 1, name='predictions')
			tf.summary.histogram('output_prediction', self.predictions)
                
		#Calculate Mean cross-entropy loss
		with tf.name_scope('loss'):
			losses = tf.nn.sigmoid_cross_entropy_with_logits(labels = self.input_y, logits = self.scores) #  only named arguments accepted
